...
public class SqliteDAO implements DataBase {
    ...
    public boolean insertEmployee(Employee employee) {
        ...
        try {
            ...
            PreparedStatement ps = conn.prepareStatement(
                    "INSERT INTO EMPLOYEE (DNI, NAME, FIRST_SURNAME, BIRTH_DATE, NATIONALITY, ROLE, EMAIL, PASSWORD)"
                            + "VALUES (?,?,?,?,?,?,?,?)");
            ps.setString(1, employee.getDni());
            ps.setString(2, employee.getName());
            ps.setString(3, employee.getFirstSurname());
            ps.setString(4, String.valueOf(employee.getBirthDate()));
            ps.setString(5, employee.getNationality());
            ps.setString(6, employee.getRole());
            ps.setString(7, employee.getEmail());
            ps.setString(8, employee.getPassword());
            updateCount = ps.executeUpdate();
            PreparedStatement ps2 = conn.prepareStatement("SELECT MAX(EMPLOYEE_ID) FROM EMPLOYEE");
            ResultSet rs = ps2.executeQuery();
            int lastId = rs.getInt(1);
            PreparedStatement ps3 = conn.prepareStatement("INSERT INTO EMPLOYEE_REGISTRATION_CANCELLATION (EMPLOYEE_ID, START_DATE) VALUES (?,?)");
            ps3.setInt(1, lastId);
            ps3.setString(2, String.valueOf(LocalDate.now()));
            updateCount2 = ps3.executeUpdate();
            ps.close();
            ps2.close();
            ps3.close();
            conn.close();
            if (updateCount + updateCount2 == 2) {
        ...
        try {
            ...
            while (rs.next()) {
                employees.add(new Employee(rs.getInt(1),
                        ...
                        rs.getString(3),
                        rs.getString(4),
                        LocalDate.parse(String.valueOf(rs.getString(5))),
                        rs.getString(6),
                        rs.getString(7),
                        rs.getString(8),
                        rs.getString(9),
    ...
    public boolean dismissEmployee(int id) {
        ...
        if (reply == JOptionPane.YES_OPTION) {
            try {
                ...
                ps2.setString(1, String.valueOf(LocalDate.now()));
                ps2.setInt(2, id);
                updateCount = ps.executeUpdate();
                ps2.executeUpdate();
                ps.close();
    ...
    public boolean reemployEmployee(int id) {
        ...
        if (reply == JOptionPane.YES_OPTION) {
            try {
                ...
                ps.setInt(2, id);
                PreparedStatement ps2 = conn.prepareStatement(
                        "INSERT INTO EMPLOYEE_REGISTRATION_CANCELLATION (EMPLOYEE_ID, START_DATE) VALUES (?,?)");
                ps2.setInt(1, id);
    ...
    public boolean insertClient(Client client) {
        ...
        try {
            ...
            PreparedStatement ps = conn.prepareStatement(
                    "INSERT INTO CLIENT (DNI, NAME, FIRST_SURNAME, SECOND_SURNAME, BIRTH_DATE, PROFESSION)"
                            + "VALUES (?,?,?,?,?,?)");
            ...
            updateCount = ps.executeUpdate();
            PreparedStatement ps2 = conn.prepareStatement("SELECT MAX(CLIENT_ID) FROM CLIENT");
            ResultSet rs = ps2.executeQuery();
            int lastId = rs.getInt(1);
            PreparedStatement ps3 = conn.prepareStatement(
                    "INSERT INTO CLIENT_REGISTRATION_CANCELLATION (CLIENT_ID, START_DATE) VALUES (?,?)");
            ps3.setInt(1, lastId);
            ps3.setString(2, String.valueOf(LocalDate.now()));
            updateCount2 = ps3.executeUpdate();
            ps.close();
    ...
    public boolean updateClient(Client client) {
        ...
        try {
            ...
            ps.setString(3, client.getFirstSurname());
            ps.setString(4, client.getSecondSurname());
            ps.setString(5, String.valueOf(client.getBirthDate()));
            ps.setString(6, client.getProfession());
    ...
    public boolean singUpClientForTour(Client client, Tour tour) {
        ...
        try {
            ...
            PreparedStatement ps2 = conn.prepareStatement("SELECT COUNT(*) FROM CLIENT_TOUR WHERE CLIENT_ID=? AND CLIENT_TOUR.TOUR_ID IN (SELECT TOUR_ID FROM TOUR WHERE STRFTIME('%Y', TOUR.START_DATE) IN (?))");
            ...
            switch (attendedToursCount) {
                case 5 -> {
                    ps3 = conn.prepareStatement("INSERT INTO BONUS (CLIENT_ID, BONUS_TYPE, DESCRIPTION, ATTAINTMENT_DATE) VALUES (?, 5, ?, ?)");
                    ps3.setInt(1, client.getId());
                    ps3.setString(2, "5% descuento");
                    ps3.setString(3, String.valueOf(LocalDate.now()));
                }
                case 10 -> {
                    ps3 = conn.prepareStatement("INSERT INTO BONUS (CLIENT_ID, BONUS_TYPE, DESCRIPTION, ATTAINTMENT_DATE) VALUES (?, 10, ?, ?)");
                    ps3.setInt(1, client.getId());
                    ps3.setString(2, "10% descuento");
                    ps3.setString(3, String.valueOf(LocalDate.now()));
                }
                case 15 -> {
                    ps3 = conn.prepareStatement("INSERT INTO BONUS (CLIENT_ID, BONUS_TYPE, DESCRIPTION, ATTAINTMENT_DATE) VALUES (?, 15, ?, ?)");
                    ps3.setInt(1, client.getId());
                ...
                case 20 -> {
                    ps3 = conn.prepareStatement("INSERT INTO BONUS (CLIENT_ID, BONUS_TYPE, DESCRIPTION, ATTAINTMENT_DATE) VALUES (?, 20, ?, ?)");
                    ps3.setInt(1, client.getId());
                    ps3.setString(2, "20% descuento");
                    ps3.setString(3, String.valueOf(LocalDate.now()));
                }
                case 25 -> {
                    ps3 = conn.prepareStatement("INSERT INTO BONUS (CLIENT_ID, BONUS_TYPE, DESCRIPTION, ATTAINTMENT_DATE) VALUES (?, 25, ?, ?)");
                    ps3.setInt(1, client.getId());
        ...
        try {
            ...
            while (rs.next()) {
                clients.add(new Client(rs.getInt(1),
                        ...
                        rs.getString(4),
                        rs.getString(5),
                        LocalDate.parse(String.valueOf(rs.getDate(6))),    // Cambiarlo en todas a getDate
                        rs.getString(7),
                        rs.getInt(8) != 0));
            }
            ps.close();
    ...
    public double getClientAnnualExpense(Client client) {
        ...
        try {
            ...
            LocalDate localDate = LocalDate.now();
            String currentYear = String.valueOf(localDate.getYear());
            PreparedStatement ps = conn.prepareStatement("SELECT T.PRICE FROM TOUR T NATURAL JOIN CLIENT_TOUR CT WHERE CT.CLIENT_ID=? AND STRFTIME('%Y', T.START_DATE)  IN (?) AND T.CANCELLED = 0");
            ps.setInt(1, client.getId());
    ...
    public int getClientAnnualDiscountPercentage(Client client) {
        ...
        try {
            ...
            LocalDate localDate = LocalDate.now();
            String currentYear = String.valueOf(localDate.getYear());
            PreparedStatement ps = conn.prepareStatement("SELECT MAX(BONUS_TYPE) FROM BONUS WHERE CLIENT_ID=? AND STRFTIME('%Y', ATTAINTMENT_DATE)  IN (?)");
            ps.setInt(1, client.getId());
            ps.setString(2, currentYear);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
    ...
    public boolean insertTour(Tour tour) {
        ...
        try {
            conn = DBConnectionUtil.getConexion();
            PreparedStatement ps = conn.prepareStatement(
                    "INSERT INTO TOUR (EMPLOYEE_ID, TITLE, DESCRIPTION, TOPIC, PLACE, STARTING_POINT, MAX_ATTENDEES, PRICE, START_DATE, CANCELLED)"
                            + "VALUES (?,?,?,?,?,?,?,?,?,?)");
            ...
            ps.setString(9, String.valueOf(tour.getStartDate()));
    ...
    public boolean updateTour(Tour tour) {
        ...
        try {
            ...
            ps.setString(1, String.valueOf(tour.getEmployee().getId()));
            ps.setString(2, tour.getTitle());
            ps.setString(3, tour.getDescription());
            ps.setString(4, tour.getTopic());
            ps.setString(5, tour.getPlace());
            ps.setString(6, tour.getStartingPoint());
            ps.setString(7, String.valueOf(tour.getMaxAttendees()));
            ps.setString(8, String.valueOf(tour.getPrice()));
            ps.setString(9, String.valueOf(tour.getStartDate()));
    ...
    public double getTourTotalRaised(Tour tour) {
        ...
        try {
            Connection conn = DBConnectionUtil.getConexion();
            PreparedStatement ps = conn.prepareStatement(" SELECT ((SELECT PRICE FROM TOUR WHERE TOUR_ID = ?) * (SELECT COUNT(*) FROM CLIENT_TOUR WHERE TOUR_ID = ?)) ");
            ps.setInt(1, tour.getId());
            ps.setInt(2, tour.getId());
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
    ...
    public ArrayList<Bonus> getAllBonussesAchievedThisYearBy(Client client) {
        ...
        try {
            ...
            LocalDate localDate = LocalDate.now();
            String currentYear = String.valueOf(localDate.getYear());
            PreparedStatement ps = conn.prepareStatement("SELECT * FROM BONUS WHERE CLIENT_ID=? AND STRFTIME('%Y', ATTAINTMENT_DATE)  IN (?)");
            ps.setInt(1, client.getId());
            ps.setString(2, currentYear);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                ...
                bonus.setAttaintmentDate(LocalDate.parse(String.valueOf(rs.getString(5))));
                bonuses.add(bonus);
            }
            ps.close();
            conn.close();
    ...
    public Integer logEmployeeClockIn(Employee employee) {
        ...
        try {
            conn = DBConnectionUtil.getConexion();
            PreparedStatement ps = conn.prepareStatement("INSERT INTO EMPLOYEE_CLOCK_IN_CLOCK_OUT (START_DATE, EMPLOYEE_ID) " +
                    "VALUES (?,?)");
            ps.setString(1, String.valueOf(Timestamp.from(Instant.now())));
            ps.setInt(2, employee.getId());
            updateCount = ps.executeUpdate();
            ps.close();
            if (updateCount == 1) {
                PreparedStatement ps2 = conn.prepareStatement("SELECT MAX(SESSION_ID) FROM EMPLOYEE_CLOCK_IN_CLOCK_OUT");
                ResultSet rs = ps2.executeQuery();
                Integer lastId = null;
                if (rs.next()) {
                    lastId = rs.getInt(1);
    ...
    public void logEmployeeClockOut(int sessionId) {
        ...
        try {
            conn = DBConnectionUtil.getConexion();
            PreparedStatement ps = conn.prepareStatement("UPDATE EMPLOYEE_CLOCK_IN_CLOCK_OUT SET END_DATE=? WHERE SESSION_ID=?");
            ps.setString(1, String.valueOf(Timestamp.from(Instant.now())));
    ...
    public ArrayList<String> getDataBaseMetaData() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                data.add(headers);
                data.add(separator);
                ResultSet rs3 = databaseMetaData.getColumns(null, null, table, null);
                while (rs3.next()) {
